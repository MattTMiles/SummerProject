...
%	As of yet our knowledge is incomplete on the exact mechanism that allows material with a high enough neutron density to escape the supernovae event.
	...
	\todo{1. Calculate mass of Eu expected form NSM within 15 kpc sphere throughout age  of  universe. 2. Calculate  mass of Eu in each 0.8 solar mass star. 3. How many we expect in that 15 kpc sphere throughout universe? 4. Multiply by fraction of LAMOST giants/number of giant stars in Milky Way, assuming star formation rate and typical IMF etc. 5. Repeat for CCSN. 6. Since estimated number from CCSN is only 4 giant stars in LAMOST, then state how long it would take to produce 60 r-process stars in LAMOST: orders of magnitude longer than the age of the universe.}
	In preparing these calculations we assumed a constant star formation rate of 2 M$_\odot {\rm yr}^{-1}$ in the disk of the Milky Way, with a typical IMF and giant star birth rate of approximately 0.3\,stars per year. We considered a spherical volume out to $r_\textrm{GC} = 15\,{\rm kpc}$ from the  centre of the Galaxy. Therefore if we assume a steady-state system, the number of red giant stars in the Milky Way comes to approximately $7.53\times10^7$. This allows us to find the percentage of the Milky Ways stars that our sample of r-process  stars represents.
	
	We assume that a star is said to be enhanced if $[{\rm Eu/Fe}] \geq 0.7$, and assert that all 47 stars here exhibit abundances exceeding this lower-bound. Taking the mass of europium ejected from a neutron star merger as $M_{\rm Eu}\approx10^{-4.5}\,M_{\odot}$, this allows us to calculate  a density of europium $\rho_{\rm Eu}$,
\begin{equation}
	\centering
	\rho_{\rm Eu} = M_\textrm{Eu}\frac{dN}{dt}(13.87\times10^9\,{\rm yr})
\end{equation}

\noindent{}where ${dN}/{dt}$ is the estimated rate of neutron star mergers (1,000$\,{\rm Gpc}^{-3}{\rm yr}^{-1}$).
